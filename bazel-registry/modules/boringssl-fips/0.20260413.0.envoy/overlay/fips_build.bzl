...
OUTPUT=$$(mktemp)
if ! $(location //:boringssl_fips.genrule_cmd) > $$OUTPUT 2>&1; then
    echo "Build failed:"
    cat $$OUTPUT >&2
    exit 1
fi
...
def _create_build_config(prefix, lib, arch, arch_alias, hermetic_sysroot):
    ...
    if lib == "libc++":
        conditions.append("//:libcxx_enabled")
    else:
        conditions.append("//:libstdcxx_enabled")

    if hermetic_sysroot:
        conditions.append("//:use_hermetic_sysroot")
    else:
        conditions.append("//:use_local_sysroot")
