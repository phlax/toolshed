"""End-to-end test: verify the built wheel METADATA for the test fixture package.
...
The test intentionally fails if the toolshed_package macro wires pinned dev deps
into the wheel instead of the per-package publish-requirements.in ranges.
"""
...
def test_wheel_requires_dist_no_extra_packages() -> None:
    """Main Requires-Dist must contain exactly the packages in publish-requirements.in.
