...
if [ -n "${TEST_SRCDIR:-}" ]; then
    ...
    COMPARE_SCRIPT="${RUNFILES_DIR}/dependency/version-compare.sh"
else
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    COMPARE_SCRIPT="$(cd "${SCRIPT_DIR}/.." && pwd)/version-compare.sh"
fi
...
assert_compare() {
    ...
    actual="$(bash "${COMPARE_SCRIPT}" compare "${left}" "${right}")"
...
assert_compare "35.1.bcr.envoy" "35.1.bcr.1" "1"
assert_compare "35.1.bcr.10" "35.1.bcr.2" "1"

sorted="$(bash "${COMPARE_SCRIPT}" sort "35.1.bcr.2" "35.1.bcr.10" "35.1.bcr.1")"
expected=$'35.1.bcr.1\n35.1.bcr.2\n35.1.bcr.10'
