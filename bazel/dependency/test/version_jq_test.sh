...
# jq filters below use `$a`/`$b`/`$versions` (jq variables bound via --arg),
# not shell expansion.
...
assert_compare() {
  ...
  actual="$(${JQ_BIN} -nr -L "${JQ_DIR}" --arg a "${left}" --arg b "${right}" 'include "version"; compare_versions($a; $b)')"
  [[ "${actual}" == "${expected}" ]] || {
...
sorted="$(${JQ_BIN} -nr -L "${JQ_DIR}" --argjson versions '["35.1.bcr.10","35.1.bcr.2","35.1.bcr.1","1.9.0-rc1","1.9.0","1.9.0.bcr.1"]' 'include "version"; $versions | sort_versions | join("\n")')"
expected=$'1.9.0-rc1\n1.9.0\n1.9.0.bcr.1\n35.1.bcr.1\n35.1.bcr.2\n35.1.bcr.10'
