...
cat > "${tmpdir}/MODULE.bazel" <<'EOF'
module(name = "live_test", version = "0.1.0")
bazel_dep(name = "bazel_skylib", version = "1.8.2")
EOF

JQ_BIN="${JQ_BIN}" bash "${UPDATE_SCRIPT}" \
    "${tmpdir}/MODULE.bazel" \
    "${tmpdir}/deps.json" \
...
"${JQ_BIN}" -e '.bazel_skylib.latest != null and (.bazel_skylib.registries | length) == 1' "${tmpdir}/report.json" >/dev/null
