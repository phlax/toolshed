...
prepare_workspace() {
    ...
    sed \
        ...
        "${DEPS_TEMPLATE}" > "${workspace}/deps.json"
    sed \
        -e "s|__REGISTRY_BCR__|${bcr_root}|g" \
        -e "s|__REGISTRY_ENVOY__|${envoy_root}|g" \
        "${BAZELRC_TEMPLATE}" > "${workspace}/.bazelrc"
...
assert_failure() {
    ...
    if BUILD_WORKSPACE_DIRECTORY="${tmpdir}" bash "${UPDATE_SCRIPT}" \
        "${tmpdir}/MODULE.bazel" \
        "${tmpdir}/deps.json" \
...
assert_failure "Dependency unknown not found in dependency metadata" unknown
