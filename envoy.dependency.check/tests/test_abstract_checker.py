...
def test_checker_constructor():
    ...
    assert checker.checks == ("cves", "dates", "issues", "releases")
...
def test_checker_cves(patches):
    ...
    with patched as (m_deps, m_config, m_class):
...
def __test_checker_dependencies(patches, failures):
    checker = DummyDependencyChecker()
    patched = patches(
        "sorted",
        "tuple",
...
def test_checker_dependency_metadata(patches):
    ...
    assert (
        list(m_path.return_value.read_text.call_args)
        == [(), {}])
